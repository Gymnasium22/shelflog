-- Stage 7: multi-entity household search (FTS + pg_trgm)

-- Ensure pg_trgm operators available on public path
...
create index if not exists items_brand_trgm_idx
  on public.items using gin (brand extensions.gin_trgm_ops);
create index if not exists items_model_trgm_idx
  on public.items using gin (model extensions.gin_trgm_ops);
create index if not exists items_serial_trgm_idx
  on public.items using gin (serial_number extensions.gin_trgm_ops);
create index if not exists items_notes_trgm_idx
  on public.items using gin (notes extensions.gin_trgm_ops);

create index if not exists boxes_code_trgm_idx
  on public.boxes using gin (code extensions.gin_trgm_ops);
create index if not exists boxes_name_trgm_idx
  on public.boxes using gin (name extensions.gin_trgm_ops);
create index if not exists boxes_notes_trgm_idx
  on public.boxes using gin (notes extensions.gin_trgm_ops);
...
create index if not exists documents_notes_trgm_idx
  on public.documents using gin (notes extensions.gin_trgm_ops);

create index if not exists storage_locations_name_trgm_idx
  on public.storage_locations using gin (name extensions.gin_trgm_ops);

-- Backfill item search vectors (in case older rows missed trigger)
update public.items
set name = name
...
begin
  ...
  -- plainto_tsquery is safe for user input; empty query handled above for pure filter cases
  if length(v_q) > 0 then
    v_tsquery := plainto_tsvector_query(v_q);
  end if;
...
-- helper was mistakenly referenced; remove dead declare usage by fixing function
-- (plainto_tsvector_query does not exist — already using plainto_tsquery above)
